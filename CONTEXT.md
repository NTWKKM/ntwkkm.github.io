...
### ADR-012: Full-Card Interactive Anchors and Threshold-Gated Drag Interactions

- **Context**: On the homepage, paper cards were rendered as `<div>` containers with only a nested link anchor. A CSS rule applied `pointer-events: none` immediately upon `mousedown` (adding `.active`), which prevented normal left clicks from executing. Right-clicking or dragging interfered with navigation.
- **Decision**: Render `.paper-card` as a top-level `<a>` element linking directly to the paper target URL (`target="_blank"`), replacing the child link with a presentational `<span>`. Remove the aggressive `pointer-events: none` CSS suppression rule. Gate drag-state activation strictly to movements exceeding 5px on left-click (`e.button === 0`), and cleanly intercept/cancel click propagation only when an actual drag occurred.
- **Consequence**: Users can click anywhere on the paper card (or the "Read Paper" prompt) to open the publication in a new tab, while horizontal mouse drag scrolling and slider navigation buttons continue to work smoothly without accidental navigation.
