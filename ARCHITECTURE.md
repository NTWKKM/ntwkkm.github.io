...
| `fray/dashboard-snapshot.json` | `[NEW]` Raw observability snapshot from Fray (updated via n8n) |
| `fray/dashboard-snapshot-clean.json` | `[NEW]` Normalized & validated snapshot (auto-generated by CI) |
...
Observability Data (Auto-Synced):
  ├── fray/index.html        → Dashboard UI (consumes clean snapshot data)
  ├── fray/dashboard-snapshot.json → Raw Fray system state metrics (pushed by n8n)
  └── fray/dashboard-snapshot-clean.json → Normalized & validated Fray metrics (auto-generated by CI)
...
Path D — fray/dashboard-snapshot.json (Workflow 5, every 8h):
  Fray/n8n commits dashboard-snapshot.json via GitHub API
    → Triggers: normalize-dashboard.yml (normalize & validate stress values)
    → Commits fray/dashboard-snapshot-clean.json ([skip ci])
    → Triggers: pages-build-deployment.yml (deploy to GitHub Pages)
```
...
- `fray/dashboard-snapshot.json` — **Raw Data.** Observability metric snapshot pushed by Fray/n8n. Triggers the normalization workflow.
- `fray/dashboard-snapshot-clean.json` — **Auto-generated.** Created by `normalize_dashboard.js` with normalized stress levels. Never edit manually.
...
graph LR
    ...
    G -->|"Triggers"| NORM["GitHub Actions<br/>normalize-dashboard.yml"]
    NORM -->|"dashboard-snapshot-clean.json"| G
    NORM -->|"Triggers"| DEPLOY["GitHub Actions<br/>pages-build-deployment.yml"]
```
...
#### 5. The Normalization (GitHub Action Pipeline)
* **Tools:** GitHub Actions (`normalize-dashboard.yml`) + Node script (`normalize_dashboard.js`)
* **Design Rationale:** The raw `dashboard-snapshot.json` is generated by a local AI agent, which may vary the JSON structure, keys, or naming conventions over time while retaining identical semantic meaning. The normalization action acts as an adapter layer: it processes, maps, and standardizes these metrics into a reliable schema (`dashboard-snapshot-clean.json`) so the frontend can display them correctly without breaking due to upstream variations.
* **Process:**
  1. A commit on `fray/dashboard-snapshot.json` triggers the `normalize-dashboard.yml` workflow.
  2. The workflow executes `normalize_dashboard.js` to parse page sizes, sanitize fields, and map raw stress values (`STRESSED` -> `WARNING`, `CRITICAL` -> `CRITICAL`) using `normalizeStressLevel`.
  3. The normalized output is written to `fray/dashboard-snapshot-clean.json`.
  4. The workflow commits the clean file back to the repository using `[skip ci]` (to prevent infinite normalizer triggering) and calls `pages-build-deployment.yml` downstream.

#### 6. The Presentation (Web Interface)
* **Tools:** GitHub Pages + `index.html`
* **Process:**
  1. Once deployed, visitors to the Fray dashboard fetch `dashboard-snapshot-clean.json` with a cache-busting query parameter.
  2. Client-side JS reads the normalized stress states and matches them with local thresholds for high-fidelity UI rendering.
...
| `normalize-dashboard.yml` | Push to `fray/dashboard-snapshot.json` or manual | `dashboard-normalization` | Normalize stress levels and telemetry metrics |
...
| `.github/scripts/normalize_dashboard.js` | Sanitize and normalize raw Fray metrics and stress indicators |
