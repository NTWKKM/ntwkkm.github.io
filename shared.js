...
function initCommandPalette() {
    ...
    <div id="cmd-palette-overlay" class="cmd-palette-overlay">
        <div class="cmd-palette" role="dialog" aria-modal="true" aria-label="Command Palette">
            <div class="cmd-palette-header">
                ...
                <input type="text" id="cmd-palette-input" class="cmd-palette-input" placeholder="Search pages, tools... (e.g., 'fray')" autocomplete="off" spellcheck="false" />
    ...
    const commands = [
        { title: 'Home', path: '/', icon: '🏠', desc: 'Main portfolio and research papers' },
        { title: 'Research Blog', path: '/blog.html', icon: '📝', desc: 'Medical research articles and notes' },
        { title: 'Fray Dashboard', path: '/fray/index.html', icon: '⚡', desc: 'Cognitive Observability Core' },
        { title: 'Package Tracker', path: '/tracking/index.html', icon: '📦', desc: 'Thailand Post tracking' },
        { title: 'GitHub Profile', path: 'https://github.com/NTWKKM', icon: '💻', desc: 'View source code and projects', external: true }
    ];
    ...
    function renderResults(query) {
        ...
        if (q) {
            filtered = commands.filter(c => c.title.toLowerCase().includes(q) || c.desc.toLowerCase().includes(q));
        }
        
        selectedIndex = 0;

        if (filtered.length === 0) {
            resultsContainer.innerHTML = '<div class="cmd-no-results">No results found for "' + escapeHTML(query) + '"</div>';
            return;
        }
    ...
    function updateSelection() {
        ...
        const selected = resultsContainer.querySelector('.cmd-item.selected');
        if (selected) {
            selected.scrollIntoView({ block: 'nearest' });
        }
    }

    function executeSelected(filtered) {
        if (filtered.length === 0) return;
        const selected = filtered[selectedIndex];
        if (selected) {
            togglePalette();
            
            // Handle root-relative paths cleanly by appending to the repo origin
